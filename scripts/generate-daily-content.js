...
function getExistingArticles() {
  try {
    const newsFilePath = path.join(__dirname, "../src/data/newsData.ts");
    if (!fs.existsSync(newsFilePath)) return [];
    const fileContent = fs.readFileSync(newsFilePath, "utf-8");
    const match = fileContent.match(/export const FINANCIAL_NEWS: NewsArticle\[\]\s*=\s*(\[[\s\S]*\]);/);
    if (match && match[1]) {
      const parsed = new Function("return " + match[1])();
      if (Array.isArray(parsed)) return parsed;
    }
  } catch (err) {
    console.warn("   ⚠️ Could not parse existing articles:", err.message);
  }
  return [];
}
...
function writeNewsData(articles) {
  ...
  // Only prepend new articles whose slug is not already present
  const toAdd = articles.filter((a) => !existingSlugs.has(a.slug));

  // Preserve archive: new articles first, then all existing historical articles up to 60 total
...
`;
  ...
  console.log(`   ✅ Saved: src/data/newsData.ts (${combined.length} total archived articles, ${toAdd.length} newly added)`);
}
